;; Based on: https://github.com/penpot/penpot/blob/de394a7d4e7e590047ce7c2647bcbc0da8d38e2a/backend/src/app/http/oauth/gitlab.clj
;; Confirmed: 1.3.0-alpha
...
(defn- build-oauth-uri
  ...
  (let [base-uri (u/uri (:base-uri cfg))]
    (assoc base-uri :path "/auth/realms/<REALM>/protocol/openid-connect/auth")))

(defn- build-token-url
  [cfg]
  (let [base-uri (u/uri (:base-uri cfg))]
    (str (assoc base-uri :path "/auth/realms/<REALM>/protocol/openid-connect/token"))))

(defn- build-user-info-url
  [cfg]
  (let [base-uri (u/uri (:base-uri cfg))]
    (str (assoc base-uri :path "/auth/realms/<REALM>/protocol/openid-connect/userinfo"))))
